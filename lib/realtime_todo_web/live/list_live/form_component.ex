defmodule RealtimeTodoWeb.ListLive.FormComponent do
  use RealtimeTodoWeb, :live_component
  ...
  def render(assigns) do
    ...
    <div>
      ...
      <.simple_form
        ...
        phx-submit="save"
      >
        <.input field={@form[:list]} type="text" label="List" />
  ...
  def update(%{list: list} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Todo.change_list(list))
     end)}
  end

  @impl true
  def handle_event("validate", %{"list" => list_params}, socket) do
    changeset = Todo.change_list(socket.assigns.list, list_params)
  ...
  defp save_list(socket, :edit, list_params) do
    case Todo.update_list(socket.assigns.list, list_params) do
      {:ok, list} ->
